    ...
    with st.form("form_login"):
        ...
        if btn_entrar:
            if email_input:
                ...
                res = supabase.table("usuarios_autorizados").select("*").eq("email", email_input.strip().lower()).execute()
                ...
                if res.data:
                    ...
                    st.error("Acesso negado. E-mail não cadastrado na equipe autorizada.")
            else:
...
# --- SISTEMA PRINCIPAL (LIBERADO APÓS LOGIN) ---
    ...
    if menu == "📝 Campo: Cadastrar Furo":
        ...
        with st.form("form_collar", clear_on_submit=True):
            ...
            if btn_salvar_furo:
                dados_collar = {
                    "hole_id": hole_id, "projeto": projeto, "alvo": alvo,
                    "utm_easting": utm_x, "utm_northing": utm_y, "elevation": elevation,
                    "azimuth": azimuth, "dip": dip, "profundidade_final": prof_final
                }
                supabase.table("furos_sondagem").insert(dados_collar).execute()
                st.success(f"Furo {hole_id} registrado por {st.session_state.usuario_nome}!")
        ...
        if not lista_furos:
            ...
            with st.form("form_intervalo", clear_on_submit=True):
                ...
                with col1:
                    ...
                    recuperacao = st.number_input("Recuperação (%)", value=95.0)
                with col2:
                    litologia = st.text_input("Litologia Dominante", value="Granito")
                    alteracao = st.text_input("Alteração Hidrotermal", value="Potássica")
                with col3:
                    mineralizacao = st.text_input("Mineralização", value="Calcopirita")
                    teor = st.number_input("Teor Principal", value=0.85, step=0.01)

                obs = st.text_area("Observações Geológicas", value="Zona mineralizada.")
                btn_salvar_intervalo = st.form_submit_button("➕ Adicionar Intervalo")

                if btn_salvar_intervalo:
                    dados_intervalo = {
                        "hole_id": furo_selecionado, "from_m": from_m, "to_m": to_m,
                        "recuperacao_pct": recuperacao, "litologia": litologia,
                        "alteracao_hidrotermal": alteracao, "mineralizacao": mineralizacao,
                        "teor_principal": teor, "observacoes": obs
                    }
    ...
    # --- MENU 3: DATA SCIENCE ---
        ...
        if df_collar.empty or df_intervalos.empty:
            st.info("Aguardando inserção de dados pela equipe autorizada.")
        else:
            col_kpi1, col_kpi2, col_kpi3 = st.columns(3)
            col_kpi1.metric("Total de Furos", len(df_collar))
            col_kpi2.metric("Teor Médio Geral", round(df_intervalos["teor_principal"].mean(), 2))
            col_kpi3.metric("Média de Recuperação", f"{round(df_intervalos['recuperacao_pct'].mean(), 1)}%")

            st.markdown("---")
            st.dataframe(pd.merge(df_intervalos, df_collar, on="hole_id", how="left"), use_container_width=True)
